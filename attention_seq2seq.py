...
def decoding_layer(target_letter_to_int, decoding_embedding_size, num_layers, rnn_size, target_sequence_length,
                   max_target_sequence_length, encoder_state, decoder_input, attention_decoder_cell):
    # 1. Embedding
    ...
    with tf.variable_scope("decode", reuse=True):
        ...
        predicting_decoder = tf.contrib.seq2seq.BasicDecoder(attention_decoder_cell, predicting_helper, initial_state, output_layer)
...
def seq2seq_model(input_data, targets, lr, target_sequence_length, max_target_sequence_length, source_sequence_length,
    ...
    def get_attention_cell(rnn_size):
        lstm_cell = tf.contrib.rnn.LSTMCell(rnn_size, initializer=tf.random_uniform_initializer(-0.1, 0.1, seed=2))
        return lstm_cell
    cell = tf.contrib.rnn.MultiRNNCell([get_attention_cell(rnn_size) for _ in range(num_layers)])
